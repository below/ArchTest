...
public struct SVKAPICardRequest: SVKAPIRequestProtocol {

    // The request data response is decoded to this type
    typealias DecodingTypeGET = SVKCard
    ...
    typealias Identity = SVKAPICardRequest

    /// The card id
    var id: String

    var absoluteQueryString: String {
        return SVKAPIClient.baseURL + "/cvi/user/api/v1/card/\(id)"
    }
    ...
    public func didDecodeData<T>(_ data: Data?, to object: inout T) {
        if let data = data {
            var card = object as! DecodingTypeGET
            card.jsonData = data
            object = card as! T
        }
    }
